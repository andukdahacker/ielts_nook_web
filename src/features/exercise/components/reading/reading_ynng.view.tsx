...
import { ReadingYNNGTask } from '../../../../schema/types';
import { ReadingComposerContext } from './reading_composer.context';
import classes from './reading_ynng.module.css';

interface ReadingYNNGViewProps {
    task: ReadingYNNGTask;
    index: number;
}

function ReadingYNNGView({ task, index }: ReadingYNNGViewProps) {
    const { removeTask, editTask } = useContext(ReadingComposerContext);
    const editInstruction = (value: JSONContent) => {
        editTask<ReadingYNNGTask>(index, {
            ...task,
            instructions: value,
        });
    };
    ...
    const markOptionCorrect = (questionIndex: number, value: 'YES' | 'NO' | 'NOT GIVEN') => {
        const newQuestions = [...task.questions];
        ...
        <>
            <Stack className={classes.container} p="xs">
                <Group justify="space-between">
                    <Text>Type: Yes, No, Not given</Text>
                    <Group>
...
export default ReadingYNNGView;
